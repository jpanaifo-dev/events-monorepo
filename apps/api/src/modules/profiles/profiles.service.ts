...
export class ProfilesService {
  ...
  async list(organizationId?: string) { const profiles = await this.prisma.profile.findMany({ where: organizationId ? { OR: [{ memberships: { some: { organizationId } } }, { participants: { some: { edition: { mainEvent: { organizationId } } } } }, { sessionSpeakers: { some: { session: { activity: { edition: { mainEvent: { organizationId } } } } } } }] } : undefined, include: { authUser: true }, orderBy: { createdAt: 'desc' } }); return profiles.map((profile) => this.serialize(profile)); }
  create(data: { id: string; firstName: string; lastName: string; phone?: string; bio?: string }) { return this.prisma.profile.create({ data: { id: data.id, firstName: data.firstName, lastName: data.lastName, phone: data.phone, bio: data.bio } }); }
