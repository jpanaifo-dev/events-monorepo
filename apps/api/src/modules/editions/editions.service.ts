...
export class EditionsService {
  ...
  create(eventId: string, data: { name: string; startDate?: string; endDate?: string; modality?: string; location?: string; description?: string; coverUrl?: string; isCurrent?: boolean; latitude?: number; longitude?: number }) { return this.prisma.edition.create({ data: { mainEventId: eventId, name: data.name, startDate: data.startDate ? new Date(data.startDate) : undefined, endDate: data.endDate ? new Date(data.endDate) : undefined, modality: data.modality || null, location: data.location || null, description: data.description || null, coverUrl: data.coverUrl || null, isCurrent: data.isCurrent ?? false, latitude: data.latitude ?? null, longitude: data.longitude ?? null } }); }
