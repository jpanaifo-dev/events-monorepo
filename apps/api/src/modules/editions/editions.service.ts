...
export class EditionsService {
  ...
  create(eventId: string, data: { name: string; startDate?: string; endDate?: string; modality?: string; location?: string }) { return this.prisma.edition.create({ data: { mainEventId: eventId, name: data.name, startDate: data.startDate ? new Date(data.startDate) : undefined, endDate: data.endDate ? new Date(data.endDate) : undefined, modality: data.modality || null, location: data.location || null } }); }
