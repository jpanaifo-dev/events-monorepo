...
export class EditionsService {
  ...
  update(id: string, data: Record<string, unknown>) { const input = { ...data }; if (typeof input.startDate === 'string') input.startDate = new Date(input.startDate); if (typeof input.endDate === 'string') input.endDate = new Date(input.endDate); return this.prisma.edition.update({ where: { id }, data: input }); }
