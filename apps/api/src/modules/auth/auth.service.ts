...
export class AuthService {
  ...
  async createAccount(data: { email: string; password: string; firstName?: string; lastName?: string; role?: 'SUPER_ADMIN' | 'ADMIN' | 'USER' }) { const id = randomBytes(16).toString('hex'); const passwordHash = await bcrypt.hash(data.password, 10); const user = await this.prisma.authUser.create({ data: { id, email: data.email, passwordHash, role: data.role ?? 'USER' } }); const profile = await this.prisma.profile.create({ data: { id, firstName: data.firstName, lastName: data.lastName } }); return { id: user.id, role: user.role, profile }; }
