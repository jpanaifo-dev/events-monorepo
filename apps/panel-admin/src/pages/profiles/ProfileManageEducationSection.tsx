...
import type { Education } from "@/store/profile.store"
...
export function ProfileManageEducationSection() {
  const { profileId } = useParams<{ profileId: string }>()
  const { selectedProfileEducation, addEducation, updateEducation, deleteEducation, profiles } = useAdminProfilesStore()
  const targetProfile = profiles.find((p) => p.id === profileId)

  useSEO({
    title: targetProfile ? `Educación de ${targetProfile.firstName}` : "Gestionar Estudios",
    description: "Administra el historial de estudios y grados académicos del usuario."
  })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await addEducation(profileId, payload)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between border-b border-border pb-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Formación educativa del usuario en la plataforma.
          </p>
        </div>
      ...
      {/* Education List */}
      {selectedProfileEducation.length === 0 ? (
        ...
        <div className="grid gap-4">
          {selectedProfileEducation.map((item) => (
            <div
              key={item.id}
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ¿Estás seguro de que deseas eliminar este estudio de "{item.title}" en "{item.institution}"? Esta acción no se puede deshacer.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
      ...
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader>
            ...
            <SheetDescription>
              Registra los datos de la institución y grado académico. Los campos con * son obligatorios.
            </SheetDescription>
          </SheetHeader>
            ...
            <FieldGroup>
              ...
              <div className="flex items-center justify-between p-1 bg-muted/10 border border-border/50 rounded-xl px-4 py-3">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Marcar si se encuentra cursando este estudio.
                  </p>
                </div>
                <Switch
                  id="currStudy"
              ...
              <div className="flex items-center justify-between p-1">
                <div className="space-y-0.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Aparecerá resaltado al inicio del perfil.
                  </p>
                </div>
                <Switch
                  id="favEdu"
