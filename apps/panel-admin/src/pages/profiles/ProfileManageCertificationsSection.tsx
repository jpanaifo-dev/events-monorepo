...
import type { Certification } from "@/store/profile.store"
...
export function ProfileManageCertificationsSection() {
  const { profileId } = useParams<{ profileId: string }>()
  const { selectedProfileCertifications, addCertification, updateCertification, deleteCertification, profiles } = useAdminProfilesStore()
  const targetProfile = profiles.find((p) => p.id === profileId)

  useSEO({
    title: targetProfile ? `Certificados de ${targetProfile.firstName}` : "Gestionar Certificaciones",
    description: "Administra el historial de certificaciones e insignias del usuario."
  })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        await addCertification(profileId, payload)
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between border-b border-border pb-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Certificaciones y licencias del usuario registradas en la plataforma.
          </p>
        </div>
      ...
      {/* Certifications list */}
      {selectedProfileCertifications.length === 0 ? (
        ...
        <div className="grid gap-4">
          {selectedProfileCertifications.map((item) => (
            <div
              key={item.id}
      ...
      <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md">
          <SheetHeader>
            ...
            <SheetDescription>
              Registra los datos de la certificación o diploma. Los campos con * son obligatorios.
            </SheetDescription>
          </SheetHeader>
