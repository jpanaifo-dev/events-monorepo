...
export function EditEditionPage() {
  ...
  useEffect(() => {
    if (edition) {
      ...
      setStatus(edition.status === "active" ? "active" : "planned")
    } else {
  ...
  const handleDelete = async () => {
    if (!eventId || !editionId) return
    if (!confirm("¿Estás seguro de que deseas eliminar permanentemente esta edición? Esta acción borrará a todos los participantes asociados y no se puede deshacer.")) {
      return
    }
    ...
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">El evento al cual pertenece esta edición no existe.</p>
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 flex-1 w-full pb-28">
        <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-10">
          <div className="space-y-1">
            ...
            <h1 className="text-3xl font-medium tracking-tight text-foreground">Editar Edición</h1>
            <p className="text-sm text-muted-foreground">
              Modifica los detalles, años o imagen de portada de la edición del evento <strong>{event.title}</strong>.
            </p>
          </div>
          <Button
            type="button"
